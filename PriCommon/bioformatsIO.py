...
def load(fns):
    ...
    """
    try:
        mrcformats = mrcIO.READABLE_FORMATS
    except NameError:
        mrcformats = []
    if isinstance(fns, basestring) and fns.endswith(mrcformats):
        return mrcIO.MrcReader(fns)
    elif isinstance(fns, basestring) and fns.endswith(bioformats.READABLE_FORMATS+OMETIFF[2:]):
        return BioformatsReader(fns)
    else:
        try:
            return mrcIO.MrcReader(fns)
        except:
...
def getWriter(out, rdr=None):
    """
    rdr: reader object
    if not rdr, use setDim() to set dimensions
    """
    try:
        mrcformats = mrcIO.WRITABLE_FORMATS
    except NameError:
        mrcformats = []
    if isinstance(out, basestring) and out.endswith(mrcformats):
        img = mrcIO.MrcWriter(out)
    elif isinstance(out, basestring) and out.endswith(bioformats.WRITABLE_FORMATS+OMETIFF[2:]):
        img = BioformatsWriter(out)
    else:
        try:
            img = mrcIO.MrcWriter(out)
        except:
...
#    factors = {'m': 0, 'mm': -3, u'\xb5'+'m': -6, 'nm': -9}
def _convertUnit(val, fromwhat='mm', towhat='um'):
    factors = {'m': 0, 'mm': -3, 'um': -6, 'nm': -9}
    factor = factors[fromwhat] - factors[towhat] or 1
...
class DynamicList(object):
    ...
    def _slice2list(self, slc):
        ...
        rlist = range(start, stop, abs(step))
        if step < 0:
            rlist.reverse()
        #print 'rlist', rlist
        return rlist
    ...
    def __setitem__(self, idx, value):
        ...
        if type(idx) == slice:
            ...
            for i in slist:#xrange(start, stop, step):
...
class AbstractChannels(DynamicList):
    ...
    def populate(self):
        if self._list:
            ...
            waves = range(WAVE_START, WAVE_END, (WAVE_END - WAVE_START)//nw)[:nw]
...
class BioformatsReader(AbstractReader, generalIO.GeneralReader):
    ...
    def readHeader(self):
        ...
        if self.fn.lower().endswith(OMETIFF) or \
          not self.fn.lower().endswith(bioformats.READABLE_FORMATS):
        ...
        if hasattr(self.omexml, 'root_node'):
            for node in self.omexml.root_node.getchildren():
                ...
                if name not in  ('Image', 'StructuredAnnotations'):
                    self.metadata[name] = dict(node.items())
                    for cnode in node.getchildren():
                        cname = repr(cnode).split()[1].split('}')[1].replace("'", '')
                        self.metadata[name][cname] = dict(cnode.items())
...
class BioformatsWriter(AbstractReader, generalIO.GeneralWriter):
    ...
    def setFromReader(self, rdr):
        ...
        if hasattr(rdr, 'omexml'):
            self.omexml = copy.deepcopy(rdr.omexml) # this does al
            self.ome = OME_XML_Editor(self.omexml)
        ...
        if self.dtype == N.uint16:
            self.imgSequence = 2 # in some reason, image order does not work for, at least, uint16
    ...
    def setup(self, arr):
       # ome = bioformats.omexml
        ...
        self.xml = self.xml.replace(u'"um"', u'"' + u'\xb5' + u'm"')
