...
#### ==== edited by Atsushi Matsuda ======
# To use structured annotations, I needed to use lxml-based omexml.py instead of buildin xml.
# There should be a better workaround, and this is just a temporal solution.
# As a result of lxml, unicode is not supported anymore.
# Also, the last line of split_qn was corrected.
# The method "iter_original_metadata" in structured annotation did not work with the newer scheme
#### =====================================
...
class OMEXML(object):
    ...
    def __init__(self, xml=None):
        ...
        self.dom = ElementTree.ElementTree(ElementTree.fromstring(xml, parser))
    ...
    class StructuredAnnotations(dict):
        ...
        def iter_original_metadata(self):
            ...
            # </StructuredAnnotations>
            #
            for annotation_node in self.node:#self.node.findall(qn(self.ns['sa'], "XMLAnnotation")):
                # <XMLAnnotation/>
                if "XMLAnnotation" not in annotation_node.tag:
                    continue
                annotation_id = annotation_node.get("ID")
                for xa_value_node in annotation_node:#.findall(qn(self.ns['sa'], "Value")):
                    # <Value/>
                    if "Value" not in xa_value_node.tag:
                        continue
                    for om_node in xa_value_node:#.findall(
                        #qn(NS_ORIGINAL_METADATA, "OriginalMetadata")):
                        # EDIT AM 20161208
                        # The old version did not work with scheme newer than 2015
                        # where tag name is "om_OriginalMetadata",
                        # not just "OriginalMetadata"
                        # <OriginalMetadata>
                        if "OriginalMetadata" not in om_node.tag:
                            continue

                        for kv in om_node:
                            if "Key" in kv.tag:
                                key_node = kv
                            elif "Value" in kv.tag:
                                value_node = kv
