...
#
# Created:     2003/11/03
# RCS-ID:      $Id: wxPyPlot.py,v 1.42 2003-10-30 20:19:54-05 gordon_williams Exp gordon_williams $
# Copyright:   (c) 2002
...
class PolyMarker(PolyPoints):
    ...
    def _square(self, dc, coords, size=1):
        ...
        xxxxxx = rect.astype(Numeric.Int32)
        dc.DrawRectangleList(xxxxxx)
...
class PlotCanvas(wx.wxWindow):
    ...
    def SetEnableGrid(self, value):
		...
		seb: use value as string to specify color"""
        #seb if value not in [True,False]:
        #seb     raise TypeError, "Value should be True or False"
    ...
    def _drawAxes(self, dc, p1, p2, scale, shift, xticks, yticks):
        ...
        if self._gridEnabled and type(self._gridEnabled) == type('black'):     # seb
            dc.SetPen(wx.wxPen(wx.wxNamedColour(self._gridEnabled),int(penWidth)))   # seb
        else:   # seb
