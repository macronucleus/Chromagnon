...
def fft(a, minCdtype=fftw.CTYPE):#N.complex64):
    """
    ...
    `a` should be a complex array,
      ...
      minCdtype
    """
    if a.dtype.type not in fftw.CTYPES:#(N.complex64, N.complex128):
        a = N.asarray(a, minCdtype)

    #import fftw
    return fftw.fft(a)
def ifft(af, normalize=True, minCdtype=fftw.CTYPE):#N.complex64):
    """
    ...
    `af` should be a complex array,
      ...
      minCdtype
    """
    if af.dtype.type not in fftw.CTYPES:#(N.complex64, N.complex128):
...
def rfft(a, minFdtype=fftw.RTYPE):#N.float32):
    ...
    `a` should be a real array,
      ...
      minFdtype
    """
    if a.dtype.type not in fftw.RTYPES:#(N.float32, N.float64):
