...
### using lxml
# 1. xml.etree -> lxml.etree
# 2. __str__ -> root = ElementTree.ElementTree(self.root_node)
#                      return ElementTree.tostring(root).decode()
# 3. parser = ElementTree.XMLParser(recover=True)
#    self.dom = ElementTree.ElementTree(ElementTree.fromstring(xml, parser))
#    remove StringIO
# 4. return split_qn returns ('','') if pattern not present
# 5. 20191127 updated with bioformats1.5.2
...
class OMEXML(object):
    ...
    def __init__(self, xml=None):
        ...
        try:
            ...
            parser = ElementTree.XMLParser(recover=True, ns_clean=True)
            self.dom = ElementTree.ElementTree(ElementTree.fromstring(xml, parser))
        except (ValueError, UnicodeEncodeError): # lxml error --> ValueError: Unicode strings with encoding declaration are not supported. Please use bytes input or XML fragments without declaration.
            xml = xml.encode("utf-8")
            self.dom = ElementTree.ElementTree(ElementTree.fromstring(xml))
