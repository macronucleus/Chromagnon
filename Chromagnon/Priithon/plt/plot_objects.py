...
fill_style_map = {'solid':wx.SOLID,'stipple': wx.STIPPLE,
                 ...
                 'diag hatch': wx.CROSSDIAG_HATCH,
                 'forward hatch':wx.FDIAGONAL_HATCH,
                 'cross hatch': wx.CROSS_HATCH,
                 'horz hatch': wx.HORIZONTAL_HATCH,
...
def get_color(in_color):
    ...
    if type(in_color) == type(''):
        ...
        color = wx.Colour(int(r),int(g),int(b))
    return color
...
def default_font():
    ...
    """
    return wx.Font(12, wx.SWISS, wx.NORMAL, 
...
class text_object(box_object,property_object):
    ...
    def size(self):
        ...
        if self.visible in ['yes','on',1] and self.text:
            ...
            #if not preset: self.dc.SetFont(wx.NullFont)
...
class text_window(wx.Window,text_object):
    """
    ...
    def OnText(self,event):
        ...
        dlg = wx.TextEntryDialog(self.GetParent(), 'New Text',
...
class axis_object(property_object):
    ...
    def layout(self,graph_area,dc):
        ...
        # #   File "X:/PrWin\scipy\plt\wxplt.py", line 143, in on_paint
        # #     self.draw(wx.PaintDC(self))
        # #   File "X:/PrWin\scipy\plt\wxplt.py", line 411, in draw
        # #     t1 = time.clock();self.reset_size(dc);t2 = time.clock()
        # #   File "X:/PrWin\scipy\plt\wxplt.py", line 359, in reset_size
        # #     self.layout_all(dc)
        # #   File "X:/PrWin\scipy\plt\wxplt.py", line 227, in layout_all
        # #     graph_area = self.layout_graph(graph_area,dc)
        # #   File "X:/PrWin\scipy\plt\wxplt.py", line 315, in layout_graph
        # #     self.y_axis.layout(graph_area,dc)
        # #   File "c:\python22\lib\site-packages\scipy\plt\plot_objects.py", line 448, in layout
        # # ZeroDivisionError: float division
        #seb scale = length / float(self.ticks[-1] - self.ticks[0])
        den = float(self.ticks[-1] - self.ticks[0])
        if den != 0:
            scale = length / den
        else:
            scale = length
    ...
    def draw_ticks(self,dc):
        if self.ticks_visible in ['yes','on']:
            ...
            pen = wx.Pen(color, self.tick_weight, style)        
    ...
    def draw_grid_lines(self,dc):
        if self.grid_visible in ['yes','on']:
            ...
            pen = wx.Pen(color,self.grid_weight, style)        
...
class axis_window(wx.Window,axis_object):
    """
    Window is currently size=0,0 window for event handling.  Text is
    NOT drawn in the window.  Maybe will do this later, but not sure
    if it is worth handling drawing th background (does GTK support
...
class poly_marker(poly_points):
    ...
    def draw(self,dc):
        if self.visible in ['on','yes']:
            ...
            symbol = self.symbol
            dc.SetPen(wx.Pen(color,weight))
            dc.SetBrush(wx.Brush(fill_color,fill_style))
...
class image_object(property_object):
    ...
    def form_image(self):
        ...
        bitmap = pixels.astype(uint8).tostring()
        image = wx.EmptyImage(self.matrix.shape[1],self.matrix.shape[0])
